<?php
/*
	Plugin Name: Paps Shipping
	Description: Paps Shipping & Delivery Tracking Integration for WooCommerce
	Version: 2.0
	Author: Paps
	Author URI: https://papslogistics.com/
  Text Domain: paps-shipping
    ...
    if ($order->status == $this->settings['delivery_submission']) {
      ...
      if (!$task_status) {
        ...
        $paramsPaps = [
          ...
          'jobPickupDatetime' => gmdate("Y-m-d G:i:s"),
...
?>
    ...
    <table class="shop_table paps_delivery">
      <tbody>
        <tr>
          <th>Livraison par:</th>
          <td><?php echo esc_html($shipping_method['name']); ?></td>
        </tr>

        <tr>
          <th>Statut de la livraison:</th>
          <td><?php echo esc_html($text_status); ?>
            <?php if ($tracking_link) { ?>
              <a target="_blank" href="<?php echo esc_url($tracking_link); ?>"> Cliquez ici</a> pour suivre la course.
            <?php } ?>
          </td>
        </tr>
